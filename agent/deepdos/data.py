...
import numpy as np
import pandas as pd
from deepdos.conf import ETC_DIR, LR_MODEL, create_logger
...
def load_model(
    has_model: bool = True, model_path: str = f"{LR_MODEL}"
) -> LogisticRegression:
    """
...
def parse_flow_data(path: str = f"{ETC_DIR}/flow_output/out.pcap_Flow.csv"):
    ...
    return {"data": data, "metadata": metadata,"metadata_dict": metadata_dict}
